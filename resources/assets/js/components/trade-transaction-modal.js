...
(function () {
  ...
  tradeTransaction.forEach(item => {
    if (item.classList.contains('trade-item-has-detail')) {
      const originalHeight = item.offsetHeight + 'px';
      ...
      item.addEventListener('click', () => {
        ...
        if (!item.classList.contains('active')) {
          requestAnimationFrame(() => {
            const actualHeight = itemDetail.scrollHeight + 80 + 'px';
            item.style.height = actualHeight;
          });
        } else {
      ...
      if (existingItem) {
        ...
        if (existingItem.getAttribute('data-status') !== trade.status) {
          ...
          usdAmount.textContent =
            trade.status === 'pending' ? '0.00$' : `${isProfit ? '+' : ''}${formatUsdBalance(trade.amount_in_usd)}$`;
          assetAmount.textContent = `${trade.status === 'pending' ? '0.00' : `${isProfit ? '+' : ''}${formatBalance(trade.amount_in_asset)}`} ${trade.asset}`;
        ...
        const tradeItemHtml = `
          <div class="transaction-item trade-transaction-item${isCompleted ? ' trade-item-has-detail' : ''}"
               ...
               data-trade-info='${trade.trade_info}'>
              ...
              <div class="d-flex flex-column align-items-end text-right">
                <span class="transaction-usd-amount ${
                  ...
                  ${
                    trade.status === 'pending'
                      ? '0.00$'
                      : `${isProfit ? '+' : ''}${formatUsdBalance(trade.amount_in_usd)}$`
                  }
                </span>
                <span style="font-size: 12px" class="transaction-asset-amount ${
                  ...
                  ${
                    trade.status === 'pending'
                      ? '0.00'
                      : `${isProfit ? '+' : ''}${formatBalance(trade.amount_in_asset)}`
                  }
                  ${trade.asset}
                </span>
              </div>
        ...
        transactionItems.appendChild(newTradeItem);
