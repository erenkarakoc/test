                  ...
                  <div class="d-flex flex-column">
                    <h6 class="mb-0">
                      ...
                      <svg class="popover-trigger text-light cursor-pointer ms-1 mb-1" data-bs-toggle="popover"
                        ...
                        data-bs-custom-class="popover-dark" data-bs-content="<div class='d-flex flex-column row-gap-2'>
                              @foreach ($notesArray as $index => $note)
<small>{{ count($notesArray) > 1 ? $index + 1 . '. ' : '' }}{{ $note }}</small>
@endforeach
                              </div>" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24">
                        <path fill="currentColor"
                          d="M22 12c0 5.523-4.477 10-10 10S2 17.523 2 12S6.477 2 12 2s10 4.477 10 10" opacity=".3" />
                        <path fill="currentColor"
                          d="M12 17.75a.75.75 0 0 0 .75-.75v-6a.75.75 0 0 0-1.5 0v6c0 .414.336.75.75.75M12 7a1 1 0 1 1 0 2a1 1 0 0 1 0-2" />
                    ...
                    <div class="d-flex align-items-center">
                      <small class="text-light">{{ \Carbon\Carbon::parse($transaction->created_at)->format('d M, Y')
                        }}</small>
                      <small @class([ 'transaction-status' , 'text-success'=> $transaction->status === 'completed',
                        'text-danger' =>
                ...
                <div class="d-flex align-items-center">
                  <div class="d-flex flex-column align-items-end text-right">
                    <span @class([ 'transaction-usd-amount' , 'text-danger'=> $transaction->type === 'sent',
                      'text-success' =>
                      $transaction->type !== 'sent' || $transaction->type !== 'locked',
                      'text-light' => $transaction->type === 'locked',
                      ])
                      class="transaction-usd-amount">{{ $transaction->type === 'locked' ? '' : ($transaction->type ===
                      'sent' ? '-' : '+') }}{{ number_format($transaction->amount_in_usd, 2) }}$</span>
              ...
              <div class="d-flex flex-column justify-content-center align-items-center text-center pb-4">
                <svg class="mt-4 mb-8" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 48 48">
                  <defs>
                    <mask id="ipTForbid0">
...
@endforeach
                ...
                <div class="d-flex align-items-center">
                  <div class="d-flex flex-column align-items-end text-right">
                    <span class="transaction-usd-amount text-success">+{{ number_format($transaction->amount_in_usd, 2)
                      }}$</span>
                    <span class="transaction-asset-amount text-light">{{ $transaction->amount_in_asset }}
                      {{ $transaction->asset }}</span>
                  </div>
...
@endforeach
                ...
                <div class="d-flex align-items-center">
                  <div class="d-flex flex-column align-items-end text-right">
                    <span class="transaction-usd-amount text-danger">-{{ number_format($transaction->amount_in_usd, 2)
                      }}$</span>
                    <span class="transaction-asset-amount text-light">{{ $transaction->amount_in_asset }}
                      {{ $transaction->asset }}</span>
                  </div>
...
@endforeach
                ...
                <div class="d-flex align-items-center">
                  <div class="d-flex flex-column align-items-end text-right">
                    <span class="transaction-usd-amount text-light">{{ number_format($transaction->amount_in_usd, 2)
                      }}$</span>
                    <span class="transaction-asset-amount text-light">{{ $transaction->amount_in_asset }}
                      {{ $transaction->asset }}</span>
                  </div>
